...
get_ipython().system(
    'wget -nc http://labfile.oss.aliyuncs.com/courses/1081/challenge-9-bike.csv')
...
# 代码开始 ### (≈ 2 行代码)
dbscan = DBSCAN(eps=0.001, min_samples=10, metric='euclidean')
dbscan_c = dbscan.fit_predict(df)
### 代码结束 ###
dbscan_c  # 输出聚类标签
...
# noise = []
# cluster = []
# for i in range(0, len(dbscan_c)):
#     if dbscan_c[i] == -1:
#         noise.append(list(df.iloc[i]))
#     else:
#         cluster.append(list(df.iloc[i]))
# noise = np.array(noise)
# cluster = np.array(cluster)
#
plt.figure(figsize=(15, 8))
df_c = pd.concat([df, pd.DataFrame(dbscan_c, columns=['c'])], axis=1)
...
plt.scatter(cluster[:, 0], cluster[:, 1], cmap='viridis', alpha=0.6)
plt.scatter(noise[:, 0], noise[:, 1], cmap='viridis', alpha=0.1)
